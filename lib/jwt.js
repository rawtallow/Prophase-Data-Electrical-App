...
export const CAN = {
  ...
  manageContracts: (role) => role === 'admin' || role === 'manager', // recurring maintenance contracts
  manageSuppliers: (role) => role === 'admin' || role === 'manager', // wholesaler trade accounts
  viewPurchaseOrders: () => true, // any role can view/draft POs; employee-drafted ones need approval before sending, same as quotes
  editPurchaseOrders: (role) => role === 'admin' || role === 'manager', // approve/reject, send, edit any PO, delete
