...
export async function countOverdueAlerts(todayStr = sydneyToday()) {
  // Cutoff computed in JS, not as `${todayStr}::date - ${DAYS}` in SQL:
  // the driver sends both sides as parameters of unknown type, and
  // `date - unknown` is ambiguous to Postgres (it could resolve to
  // date-minus-integer or date-minus-date), so it can fail at plan time.
  // One pre-resolved date parameter sidesteps the inference entirely.
  const cutoff = new Date(todayStr + 'T00:00:00');
  cutoff.setDate(cutoff.getDate() - INVOICE_OVERDUE_DAYS);
  const invoiceCutoff = cutoff.toISOString().slice(0, 10);
  ...
  const rows = await sql`
    select (
      ...
      + (select count(*) from jobs where amount_invoiced > amount_paid
           and coalesce(completed_date, created_date) < ${invoiceCutoff})
