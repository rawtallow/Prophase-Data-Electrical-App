...
// yyyy-mm-dd for "today" in the business's own timezone. Vercel's Node
// runtime defaults to TZ=UTC, so `new Date()`'s local getters (or
// toISOString()) return the UTC calendar date — for roughly 10 hours a day
// (Sydney midnight to ~10am AEST/11am AEDT) that's still "yesterday" from
// Sydney's perspective. Intl's timeZone option resolves the date in that
// zone regardless of the host process's own TZ setting.
